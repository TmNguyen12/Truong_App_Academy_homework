...
def top_titles
  # get movie titles from movies with scores greater than or equal
  # to 9 hint: use 'select' and 'where'
...
def below_average_years
  ...
  Movie.select('yr','COUNT(score) AS bad_movies')
    .where("score < 5")
    .group("yr")
...
def uma_movies
  ...
  Actor.select('movies.id','movies.title','movies.yr')
    .joins(:movies)
